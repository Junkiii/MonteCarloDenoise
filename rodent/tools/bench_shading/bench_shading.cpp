...
int main(int argc, char** argv) {
    ...
    for (size_t geom = 0, cur = 0; geom < num_geometries; ++geom, cur += rays_per_geom) {
        ...
        for (size_t i = 0; i < rays_per_geom; ++i) {
            ...
            primary_in.diffuse_r[cur + i] = 1.0f;
            primary_in.diffuse_g[cur + i] = 1.0f;
            primary_in.diffuse_b[cur + i] = 1.0f;
            primary_in.specular_r[cur + i] = 1.0f;
            primary_in.specular_g[cur + i] = 1.0f;
            primary_in.specular_b[cur + i] = 1.0f;
    ...
    if (!sorted) {
        ...
        for (size_t i = 0; i < num_rays; ++i) {
            ...
            primary_out.diffuse_r[i] = primary_in.diffuse_r[ids[i]];
            primary_out.diffuse_g[i] = primary_in.diffuse_g[ids[i]];
            primary_out.diffuse_b[i] = primary_in.diffuse_b[ids[i]];
            primary_out.specular_r[i] = primary_in.specular_r[ids[i]];
            primary_out.specular_g[i] = primary_in.specular_g[ids[i]];
            primary_out.specular_b[i] = primary_in.specular_b[ids[i]];
