...
struct Interface {
    ...
    struct DeviceData {
        ...
        anydsl::Array<float> d_diff_pixels;
        anydsl::Array<float> d_spec_pixels;
    };
    ...
    anydsl::Array<float> diff_pixels;
    anydsl::Array<float> spec_pixels;
    size_t film_width;
    size_t film_height;
    ...
    Interface(size_t width, size_t height)
        ...
        , diff_pixels(width * height * 3)
        , spec_pixels(width * height * 3)
    {}
    ...
    anydsl::Array<float>& cpu_primary_stream(size_t size) {
        return resize_array(0, cpu_primary, size, 32);
    }
    ...
    anydsl::Array<float>& gpu_first_primary_stream(int32_t dev, size_t size) {
        return resize_array(dev, devices[dev].first_primary, size, 32);
    }

    anydsl::Array<float>& gpu_second_primary_stream(int32_t dev, size_t size) {
        return resize_array(dev, devices[dev].second_primary, size, 32);
    }
    ...
    void present(int32_t dev) {
        ...
        anydsl::copy(devices[dev].d_diff_pixels, diff_pixels);
        anydsl::copy(devices[dev].d_spec_pixels, spec_pixels);
    ...
    void clear() {
        ...
        std::fill(diff_pixels.begin(), diff_pixels.end(), 0.0f);
        std::fill(spec_pixels.begin(), spec_pixels.end(), 0.0f);
        ...
        for (auto& pair : devices) {
            ...
            auto& device_diffuse = devices[pair.first].d_diff_pixels;
            auto& device_specular = devices[pair.first].d_spec_pixels;
            ...
            if (device_diffuse.size())
                anydsl::copy(diff_pixels, device_diffuse);
            if (device_specular.size())
                anydsl::copy(spec_pixels, device_specular);
        }
    }
};
...
float* get_diff_pixels() {
    return interface->diff_pixels.data();
}

float* get_spec_pixels() {
    return interface->spec_pixels.data();
}
...
inline void get_primary_stream(PrimaryStream& primary, float* ptr, size_t capacity) {
    ...
    primary.diffuse_r = ptr + 25 * capacity;
    primary.diffuse_g = ptr + 26 * capacity;
    primary.diffuse_b = ptr + 27 * capacity;
    primary.specular_r = ptr + 28 * capacity; 
    primary.specular_g = ptr + 29 * capacity;
    primary.specular_b = ptr + 30 * capacity;
     TODO: check the hardcode values
    primary.depth     = (int*)ptr + 31 * capacity;
    primary.size = 0;
}
...
void rodent_get_film_data(int32_t dev, float** pixels, float** alb_pixels, float** nrm_pixels, float** diff_pixels, float** spec_pixels, int32_t* width, int32_t* height) {
    if (dev != 0) {
        ...
        if (!device.film_pixels.size()) {
            ...
            auto diff_data = reinterpret_cast<float*>(anydsl_alloc(dev, sizeof(float) * film_size));
            device.d_diff_pixels = std::move(anydsl::Array<float>(dev, diff_data, film_size));
            anydsl::copy(interface->diff_pixels, device.d_diff_pixels);

            auto spec_data = reinterpret_cast<float*>(anydsl_alloc(dev, sizeof(float) * film_size));
            device.d_spec_pixels = std::move(anydsl::Array<float>(dev, spec_data, film_size));
            anydsl::copy(interface->spec_pixels, device.d_spec_pixels); 
        ...
        *diff_pixels = device.d_diff_pixels.data();
        *spec_pixels = device.d_spec_pixels.data();
        ...
        *diff_pixels = interface->diff_pixels.data();
        *spec_pixels = interface->spec_pixels.data();
...
void rodent_cpu_get_primary_stream(PrimaryStream* primary, int32_t size) {
    ...
    get_primary_stream(*primary, array.data(), array.size() / 32);
}
